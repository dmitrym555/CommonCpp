...
void jformat(string& outstr, const char* fmt, ...)
{
    va_list va;
    va_start(va, fmt);
...
std::string pathThisProc(const char* replfn)
{
    std::string path;
    char buf[1024];
    ssize_t ret = -1;//readlink("/proc/self/exe", buf, sizeof(buf) - 1);
    if (ret != -1)
    {
        buf[ret] = '\0';
        path.assign(buf, ret);
...
void Parser::setError(const char* msg)
{
    if (!mErr)
    {
        ...
        std::string tmsg(msg);
        tmsg.resize(48);
        jformat(mErrMsg, "Parser error: %s at line %d", tmsg.c_str(), mLineNum);
        mErr = true;
    }
}
...
void Parser::expectErr(char c, const char* str)
{
    ...
    jformat(s, "Expecting [%s] but found [%s] ", str, c_str());
    setError(s.c_str());
}
