...
HttpRequest HttpRequest::string_to_request(const std::string_view& request_string) {
    ...
    size_t argPos = path.find("?");
    if ( argPos == std::string::npos )
        argPos = path.length();
    while ( argPos < path.length() ) {
        argPos += 1;
        size_t argSplitPos = path.find( "=", argPos );
        if ( argSplitPos == std::string::npos )
            argSplitPos = path.length();

        size_t argEndPos = path.find( "&", argSplitPos );
        if ( argEndPos == std::string::npos )
            argEndPos = path.length();

        std::string argName = path.substr( argPos, argSplitPos - argPos );

        argSplitPos = std::min( argSplitPos, path.length() - 1 );

        std::string argVal = path.substr( argSplitPos + 1, argEndPos - argSplitPos + 1 );
        request.urlArgs[argName] = argVal;
        argPos = argEndPos;
    }
...
void HttpResponse::string_to_response(const std::string_view& request_string) {
    std::string start_line, header_lines, message_body;
    std::istringstream iss;
    std::string line, method, path, version;  // used for first line
    std::string key, value;                   // used for header fields
    ...
    //resp.SetUri(Uri(path));
    //if (string_to_version(version) != resp.version()) {
    //    throw std::logic_error("HTTP version not supported");
    //}
